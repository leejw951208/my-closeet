---
name: feature-verify
description: spec.md, plan.md 기반으로 코드 리뷰, 기능 검증, 보안 감사를 수행하고 docs/features/<slug>/review.md를 생성한다. 사용 예. /feature-verify user-login
---
...
- `$ARGUMENTS` 가 있으면 그대로 사용한다. 예: `/feature-verify user-login`
...
> `docs/features/$ARGUMENTS/` 문서를 찾을 수 없습니다. `/feature-plan` 을 먼저 실행하세요.
...
`/clear` 로 세션을 초기화한 뒤 `/feature-patch $FEATURE_SLUG` 로 보강하고, 이후 `/feature-verify $FEATURE_SLUG` 를 재실행하세요.
```
...
다른 기능을 계획하려면 `/clear` 로 세션을 초기화한 뒤 `/feature-plan <기능 설명>` 을 실행하세요.
```
