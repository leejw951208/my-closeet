...
1. `Skill` 툴로 `feature-verify` 를 호출해 `$FEATURE_SLUG` 를 검증한다.
2. `docs/features/$FEATURE_SLUG/review.md` 의 `OPEN` 항목 수집.
3. `OPEN` 0건이면 성공 종료.
4. `OPEN` 1건 이상이면 `Skill` 툴로 `feature-patch` 를 호출해 `$FEATURE_SLUG` 를 보강한다. `INCLUDE_APPENDIX=1` 이면 `--include-appendix` 를 인자에 포함한다.
...
`Skill` 툴로 `feature-verify` 를 호출해 `$FEATURE_SLUG` 를 검증한다.
...
OPEN 항목이 있으면 `Skill` 툴로 `feature-patch` 를 호출한다.
